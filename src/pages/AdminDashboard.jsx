...
export default function AdminDashboard() {
  ...
  const handleDelete = async (deletedId) => {
    try {
      ...
      setAuthToken(adminToken);
      await API.delete(`/events/${deletedId}`);
  ...
  return (
    <div className="max-w-6xl mx-auto p-4">
      ...
      <form onSubmit={createOrUpdateEvent} className="grid grid-cols-2 gap-2 mb-6">
        ...
        <button className="col-span-2 bg-indigo-600 text-white p-2 rounded mt-2">{editingEvent ? 'Update Event' : 'Create Event'}</button>
      </form>

      <div className="grid md:grid-cols-3 gap-4">
        {events.map(ev => (
          <EventCard key={ev._id} event={ev} isAdmin={true} onDelete={handleDelete} onEdit={handleEdit}/>
        ))}
      </div>
    </div>
  );
